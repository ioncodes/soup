...
vec3 camPos() { return vec3(0.0, 0.0, time * 4.0); }
...
vec3 glowColour() { return palette(time * 0.1, CA, CB, CC, CD); }
...
float pillarLights(vec3 rp, float r) {
  float t = sdCapsule(rp - vec3(0.0, 1.5, -3.0), vec3(-3.0, 0.0, 0.0),
                      vec3(3.0, 0.0, 0.0), r);
  t = min(t, sdCapsule(rp - vec3(0.0, 1.5, 3.0), vec3(-3.0, 0.0, 0.0),
                       vec3(3.0, 0.0, 0.0), r));
  t = min(t, sdCapsule(rp - vec3(-3.0, 1.5, 0.0), vec3(0.0, 0.0, -3.0),
                       vec3(0.0, 0.0, 3.0), r));
  t = min(t, sdCapsule(rp - vec3(3.0, 0.0, 3.0), vec3(0.0, 1.5, 0.0),
                       vec3(0.0, -1.5, 0.0), r));
  t = min(t, sdCapsule(rp - vec3(3.0, 0.0, -3.0), vec3(0.0, 1.5, 0.0),
                       vec3(0.0, -1.5, 0.0), r));
  t = min(t, sdCapsule(rp - vec3(-3.0, 0.0, 3.0), vec3(0.0, 1.5, 0.0),
  ...
  t = min(t, sdCapsule(rp - vec3(-3.0, 0.0, -3.0), vec3(0.0, 1.5, 0.0),
                       vec3(0.0, -1.5, 0.0), r));
  return min(t, sdCapsule(rp - vec3(3.0, 1.5, 0.0), vec3(0.0, 0.0, -3.0),
                          vec3(0.0, 0.0, 3.0), r));
}
...
vec4 nearestSphere(vec3 rp) {
  ...
  float z = camPos().z;
  z = z - mod(z, PARTITION_SIZE) -
      mod(time * 10.0, PARTITION_SIZE);  // start partitioning behind camera
...
vec3 bump(vec3 rp, vec3 n) {
  vec2 e = vec2(EPS, 0.0);
  float nz = noise(rp);
  vec3 d = vec3(noise(rp + e.xyy) - nz, noise(rp + e.yxy) - nz,
                noise(rp + e.yyx) - nz) /
           e.x;
...
void surfaceDetail(vec3 ro, vec3 rd, inout Scene scene) {
  ...
  if (scene.id == ROOF) {
    scene.n = bump((rp + time * 0.4) * 4.0, scene.n);
  }
...
void main() {
  ...
  out_color = vec4(sqrt(clamp(pc * 1.0, 0.0, 1.0)), 1.0);
}
